const DriveGuidstorageModule = function (fvdSynchronizer) {
  var self = this;
  ...
  function store() {
    fvdSynchronizer.localStorage.setItem(activeKeyName, JSON.stringify(id_GlobalId));
    needWrite = false;
  }
  ...
  function read() {
    id_GlobalId = {};
    globalId_id = {};
    if (fvdSynchronizer.localStorage.hasItem(TEMP_KEY_NAME)) {
      return self.applyMassGuidsChange();
    }
    try {
      id_GlobalId = JSON.parse(fvdSynchronizer.localStorage.getItem(KEY_NAME));
      makeGlobalId_id();
    } catch (ex) {}
  }
  ...
  this.applyMassGuidsChange = function () {
    ...
    id_GlobalId = JSON.parse(fvdSynchronizer.localStorage.getItem(TEMP_KEY_NAME));
    fvdSynchronizer.localStorage.removeItem(TEMP_KEY_NAME);
    makeGlobalId_id();
    this.store();
  };
...
export default DriveGuidstorageModule;
