var bookmarksListeners = {
  ...
  _prepareRemovedBookmarks: function (callback) {
    ...
    fvdSynchronizer.Utils.Async.chain([
      function (next) {
        // make bookmarks hash
        fvdSynchronizer.bookmarks.manager.getAllBookmarks(true, function (ids) {
      ...
      function () {
        ...
        ids.forEach(function (id) {
          if (!bookmarksIdsHash[id]) {
            var globalId = fvdSynchronizer.bookmarks.manager.getGlobalId(id);
            if (globalId) {
              if (!fvdSynchronizer.bookmarks.manager.isRootFolder(globalId)) {
                totalRemovedCount++;
  ...
  init: function () {
    ...
    chrome.bookmarks.onCreated.addListener(function (id) {
      ...
      fvdSynchronizer.Utils.Async.chain([
        function () {
          fvdSynchronizer.Driver.Bookmarks.Backup.setHasChanges();
          var globalId = fvdSynchronizer.bookmarks.database.setGuid(id, null);
          if (globalId) {
...
export default bookmarksListeners;
