...
// visible globally
let CHROME_MENU_ID = null;
let CHROME_TOOLBAR_ID = null;
let CHROME_UNSORTED_ID = null; // for opera only
...
var chromeBookmarks = new (function () {
  ...
  var actions = ['removeTree', 'create', 'update', 'move', 'getTree', 'getSubTree'];
...
var BookmarksManager = function (fvdSynchronizer) {
  var self = this;

  function createGuidsForAll(callback, progressCallback) {
    fvdSynchronizer.bookmarks.manager.getAllBookmarks(true, function (bookmarks) {
      var processedCount = 0;
      bookmarks.forEach(function (bookmark) {
        var guid = fvdSynchronizer.bookmarks.database.getGuid(bookmark.id);
        processedCount++;
        if (progressCallback) {
          progressCallback(processedCount, bookmarks.length);
        }
        if (!guid) {
          fvdSynchronizer.bookmarks.database.setGuid(bookmark.id, null);
        }
      });
      callback();
    });
  ...
  function getGlobalId(id) {
    ...
    return fvdSynchronizer.bookmarks.database.getGuid(id);
  }
  ...
  function getId(guid, params) {
    ...
    return fvdSynchronizer.bookmarks.database.getId(guid);
  }
    ...
    bookmarkExists(id, function (exists) {
      fvdSynchronizer.Utils.Async.chain([
        function (chainCallback) {
          if (exists) {
            fvdSynchronizer.bookmarks.listeners.disable();
            via.removeTree(id, function () {
              fvdSynchronizer.bookmarks.listeners.enable();
              chainCallback();
            });
          } else {
            chainCallback();
      ...
      function (chainCallback) {
        if (!tree.children || tree.children.length === 0) {
          ...
          fvdSynchronizer.Utils.Async.arrayProcess(
            tree.children,
            function (subTree, arrayProcessCallback) {
  ...
  this.isRootFolder = function (globalId) {
    if (fvdSynchronizer.bookmarks.database.systemGuids().indexOf(globalId) != -1) {
      return true;
    }
  ...
  this.changeGlobalId = function (oldGuid, newGuid) {
    ...
    if (id) {
      fvdSynchronizer.bookmarks.database.setGuid(id, newGuid);
    }
  };
  ...
  this.getAllGuids = function () {
    return fvdSynchronizer.bookmarks.database.getAllGuids();
  };
  ...
  this.removeAllBookmarks = function (callback, params, progressCallback) {
    ...
    this.getAllBookmarks(false, function (bookmarks) {
      fvdSynchronizer.Utils.Async.arrayProcess(
        bookmarks,
        function (bookmark, arrayProcessCallback) {
          countRemoved++;
          if (progressCallback) {
  ...
  this.getAllBookmarks = function (onlyIds, callback) {
    ...
    chrome.bookmarks.getTree(function (tree) {
      ...
      fvdSynchronizer.Utils.Async.arrayProcess(
        tree.children,
        function (rootDir, arrayProcessCallback) {
    ...
    fvdSynchronizer.Utils.Async.chain([
      function (chainCallback) {
        ...
        if (_parentId === null) {
          ...
          // ignore
          if (
            fvdSynchronizer.bookmarks.virtualBookmarks.getSubTree(bookmark.parent_id) ||
            fvdSynchronizer.bookmarks.virtualBookmarks.getSubTree(bookmark.global_id)
          ) {
            // bookmark can be saved in virtual bookmarks
            fvdSynchronizer.bookmarks.virtualBookmarks.save(bookmark);
          }
          ...
          fvdSynchronizer.Utils.Async.chain([
            function (chainCallback2) {
              ...
              if (bookmark.type == 'folder') {
                ...
                if (
                  !fvdSynchronizer.Utils.isIdenticalUrls(currentBookmark.url, bookmark.url) ||
          ...
          fvdSynchronizer.Utils.Async.chain([
            function (chainCallback2) {
              ...
              via.create(saveFields, function (_newBookmark) {
                ...
                if (_newBookmark) {
                  fvdSynchronizer.bookmarks.database.setGuid(_newBookmark.id, bookmark.global_id);
    ...
    fvdSynchronizer.Utils.Async.chain([
      ...
      function (chainCallback) {
        ...
        itemsStored.forEach(function (bookmark) {
          if (fvdSynchronizer.bookmarks.virtualBookmarks.getSubTree(bookmark.global_id)) {
            if (bookmark.type == 'folder') {
              // get all children
              var children = fvdSynchronizer.bookmarks.virtualBookmarks.getChildrenFlat(
                bookmark.global_id
              );
            ...
            fvdSynchronizer.bookmarks.virtualBookmarks.removeTree(bookmark.global_id);
          }
        });
      ...
      function (chainCallback) {
        ...
        fvdSynchronizer.Utils.Async.arrayProcess(
          itemsNotStored,
          function (bookmark, apNext) {
            // if parent of this bookmark in virtual tree, need to move bookmark to it
            // and remove from main tree
            if (!fvdSynchronizer.bookmarks.virtualBookmarks.getSubTree(bookmark.parent_id)) {
              return apNext();
            }
            ...
            fvdSynchronizer.Utils.Async.chain([
              ...
              function (next) {
                ...
                for (var i = 0; i != bookmarksToSave.length; i++) {
                  fvdSynchronizer.bookmarks.virtualBookmarks.save(bookmarksToSave[i]);
                }
                next();
              },
              function () {
  ...
  this.setListeners = function () {
    fvdSynchronizer.bookmarks.listeners.init();
  };
